...
def test_redirect_link_not_found(client):
    ...
    assert resp_json['message'] == 'Short URL not found.'
...
def test_redirect_link_inactive(client, link_factory):
    ...
    resp_json = response.json()
    assert resp_json['success'] is False
    assert resp_json['message'] == 'This short URL is inactive.'
...
# @pytest.mark.django_db
# def test_redirect_link_password_invalid(client, link_factory):
#     link = link_factory(password='abc123')
#     url = reverse('links:redirect_link', kwargs={'slug': link.slug})
#     response = client.get(url, {'password': 'wrong'})
#     assert response.status_code == 403
#     resp_json = response.json()
#     assert resp_json['success'] is False
#     assert resp_json['message'] == 'Invalid password.'
...
#     link = link_factory(password='abc123')
#     url = reverse('links:redirect_link', kwargs={'slug': link.slug})
#     response = client.get(url, {'password': 'abc123'})
#     assert response.status_code == 302
#     assert response['Location'] == link.original_url
#     link.refresh_from_db()
